...
public class Tableau {
    ...
    public void addValue(Integer value) {
        ...
        /*adding to occupied tableau
         */
        Integer a = addToRow(smallest, value);
        /*if a number is returned, i.e. a number is bumped out of the row:
         *attempt to add number to rows below
         */
        if (a != null){
            for(Cell i = smallest; i != null; i = i.below){
                if(i.below == null){ //bottom row
                    Cell newCell = new Cell(a);
                    i.below = newCell; //assigning link to newCell
                    newCell.above = i; //assigning link of newCell
                    return;
                } else {
                    addToRow(i.below, a); //adding to present row
                    if(a == null){
                        return; //break once all numbers are in place
                    }
                }
            }
        }
    ...
    protected Integer addToRow(Cell curr, int value) {
        ...
        int prev = 0;
        Cell lastCell = null;
        for (Cell i = curr; i != null; i = i.right){
            if (i.value > value){
                prev = i.value;
                i.value = value;
                return prev;
        ...
        if(lastCell.above != null){
            if(lastCell.above.right != null){
